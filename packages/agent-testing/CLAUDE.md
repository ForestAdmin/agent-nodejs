...
- **`ForestServerSandbox`** (`forest-server-sandbox.ts`) — a raw `node:http` server that mocks the Forest API endpoints the agent calls at boot/runtime: schema upload (`/agent-schema`), OIDC discovery/registration (`/oidc/*`), event subscription, IP whitelist, and crucially `/liana/v4/permissions/environment`. It derives V4 permissions on the fly by walking the uploaded `ForestSchema` and applying any `PermissionsOverride` pushed to `/permission-override`. One sandbox serves multiple agents — caches are keyed by the `forest-secret-key` header.

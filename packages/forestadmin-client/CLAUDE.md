...
- **Permission/scope logic operates on hashed action identifiers**, not raw names (`generate-action-identifier.ts`, `hash-chart.ts`, `object-hash`). Charts and segment queries are matched by hash, so a chart that "looks identical" but differs in serialization will miss the permission cache.

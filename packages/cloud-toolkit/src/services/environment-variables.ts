...
export function validateServerUrl(serverUrl: string): void {
  if (!serverUrl) {
    throw new CommandError('Missing FOREST_SERVER_URL. Please check your .env file.');
  }
...
export function validateEnvironmentVariables(env: EnvironmentVariables): void {
  if (!env.FOREST_ENV_SECRET) {
    throw new CommandError('Missing FOREST_ENV_SECRET. Please check your .env file.');
  }

...
// ─── Provider contract ───────────────────────────────────────────────────────
// Every provider must pass the exact same test suite.

const providers = [
  {
    label: 'OpenAI',
    describeProvider: describeWithOpenAI,
    aiConfig: {
      name: 'test',
  ...
  {
    label: 'Anthropic',
    describeProvider: describeWithAnthropic,
    aiConfig: {
      name: 'test',
...
providers.forEach(
  ({ label, describeProvider, aiConfig, invalidApiKey, authErrorPattern, fetchModels }) => {
    describeProvider(`${label} Integration (real API)`, () => {
      ...
      describe('route: ai-query', () => {
        it('should complete a simple chat request', async () => {
          ...
          expect(response).toMatchObject({
            ...
            model: expect.stringContaining(aiConfig.model),
      ...
      describe('error handling', () => {
        it('should throw authentication error with invalid API key', async () => {
          ...
          ).rejects.toThrow(authErrorPattern);
        }, 10000);
      ...
      describe('Model tool support verification', () => {
        ...
        beforeAll(async () => {
          modelsToTest = await fetchModels();
        });

        it('should have found models from API', () => {
...
// ─── Router-level tests (provider-independent, run once with OpenAI) ─────────

describeWithOpenAI('Router integration tests', () => {
  const router = new Router({
    aiConfigurations: [
      {
        name: 'test-gpt',
  ...
  describe('AI configuration selection', () => {
  ...
  describe('MCP Server Integration', () => {
    ...
    describe('MCP error handling', () => {
      it('should continue working when one MCP server is unreachable and log the error', async () => {
        ...
        const mixedConfig = {
          configs: {
            calculator: mcpConfig.configs.calculator,
            broken: {
    ...
    describe('route: ai-query (with MCP tools)', () => {
      it('should allow LLM to call MCP tools', async () => {
        const response = (await router.route({
          route: 'ai-query',
          body: {
            messages: [

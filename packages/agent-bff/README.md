...
| `BFF_AGENT_TIMEOUT_MS`| no      | How long the BFF waits for the customer agent on any data or action call, in milliseconds. Defaults to `10000`. Past it the route answers `504 agent_timeout`; a refused connection or an unresolvable host answers `502 network_error` instead, whenever it fails. A malformed value (non-integer, `0`, or above 2147483647) fails the boot, unlike an absent one which takes the default. |
...
connection or whose host does not resolve answers `502 network_error`. The deadline is armed when the
request starts, so a host that accepts nothing and never resets is a `504` too — `502` means the
transport failed outright, not that it ran out of time.

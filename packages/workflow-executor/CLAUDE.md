...
- **MCP tool-selection reasoning** — `McpStepExecutor.selectTool` augments each candidate tool's schema with a top-level `reasoning` field before binding (`withReasoningField`), so the model explains its choice inside the same forced (`tool_choice: 'any'`) call — no extra AI round-trip. Augmentation handles both schema shapes: zod schemas (Forest connectors, e.g. Zendesk) via `.extend()`, JSON Schema (MCP servers) via property injection — detected by duck-typing (`typeof schema.extend === 'function'`), never `instanceof` (ai-proxy and workflow-executor resolve different zod instances). The reasoning is stripped from the args before invoking the real tool (whose schema is untouched) and persisted as `McpStepExecutionData.toolSelectionReasoning` — privacy-sensitive, stays client-side, never sent in the `StepOutcome`. It survives the confirmation flow (saved with `pendingData` at Branch C, carried via `...existingExecution` at Branch A). Mirrors the `condition` executor's `reasoning` field.

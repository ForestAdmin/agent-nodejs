...
- **Idempotency in mutating executors** — `update-record`, `trigger-action`, and `mcp` executors protect against duplicate side effects via a write-ahead log in the `RunStore`, with two layers. (1) **Atomic claim before the side effect**: each mutating executor calls `claimForExecution(seed)` (in `BaseStepExecutor`) immediately before the mutation, which delegates to `RunStore.claimStepExecution` — an atomic compare-and-set that transitions the step into `idempotencyPhase: 'executing'` only if currently unclaimed. `'won'` → proceed; `'done'` → replay the success outcome; `'executing'` → throw `StepStateError`. This is the **only cross-instance dedup** (the in-memory `inFlightRuns` registry does not dedup across replicas), so the claim MUST be atomic against concurrent callers — `DatabaseStore` relies on `SELECT … FOR UPDATE` (PG/MySQL) plus the unique `(run_id, step_index)` index for the insert race; `InMemoryStore` is atomic by Node's single-threaded model. The claim runs **before** `AgentWithLog.createPending`, so the `update`/`action` activity log fires at most once. (2) **Early `checkIdempotency()` hook** (runs before `doExecute()`): a cheap read that replays `done` / rejects `executing` before redoing AI work — an optimization, not the correctness boundary. After the side effect, the executor saves `idempotencyPhase: 'done'` with the `executionResult`. Tradeoff: the claim now precedes the log (no more `beforeCall` thunk), so a failure between claim and success leaves an orphan `executing` marker → manual retry (fail-safe: never double-applies; the activity-log port retries transient failures internally). Non-mutating executors (`condition`, `read-record`, `guidance`, `load-related-record`) neither claim nor override `checkIdempotency()` — replaying them is safe.

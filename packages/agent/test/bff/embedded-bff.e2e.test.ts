...
function itServesTheDataContract(post: () => Post) {
  ...
  describe('the record contract', () => {
    it('should carry the flat id as a string while __forest.primaryKey holds it typed', async () => {
      const response = await post()('/agent/v1/people/list', { projection: ['id'] });

      expect(response.status).toBe(200);
      expect(response.body.data).toEqual([
        { id: '8', __forest: { collection: 'people', primaryKey: { id: 8 } } },
      ]);
    });

    it('should return a snake_case column under its camelCase key, while projecting its schema name', async () => {
      const response = await post()('/agent/v1/people/list', {
    ...
    it(
      ...
      async () => {
        ...
        try {
          ...
          expect(document.body.servers[0].url).toBe('/api/bff');

...
**Decorators are the extension mechanism.** `DataSourceDecorator` wraps a child datasource and lazily wraps each child collection in a `CollectionDecorator` subclass (one `WeakMap`-cached instance per child). `CollectionDecorator` (`src/decorators/collection-decorator.ts`) delegates every method to `childCollection`; the filter-bearing methods (`execute`, `getForm`, `list`, `update`, `delete`, `aggregate`) first run their filter through `refineFilter`, while `create`/`renderChart` (no filter) delegate verbatim. `refineFilter` and `refineSchema` are the only override hooks subclasses touch. Schema is cached in `lastSchema` and invalidated via `markSchemaAsDirty`, which propagates **upward** through the decorator stack (a parent patches its child's `markSchemaAsDirty` in the constructor, since children hold no parent reference). The customization layer in `datasource-customizer` is a tower of such decorators.

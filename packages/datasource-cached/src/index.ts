...
import { CachedCollectionSchema, CachedDataSourceOptions, DumpOptions } from './types';
...
function createCachedDataSource(options: CachedDataSourceOptions): DataSourceFactory {
  return async (logger: Logger) => {
    let schema = await (typeof options.schema === 'function' ? options.schema() : options.schema);

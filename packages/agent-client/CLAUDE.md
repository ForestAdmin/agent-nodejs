...
1. **`HttpRequester`** (`http-requester.ts`) is the only thing that touches the network (superagent). It bearer-auths, defaults the query to the requester's `options.timezone` or `Europe/Paris` when unset (a caller-supplied `query.timezone` still overrides both; `createRemoteAgentClient` threads its `timezone` param into that option), JSON:API-deserializes responses into camelCase (falling back to raw body/text), and on HTTP errors throws a typed `AgentHttpError(status, body, responseText)`. Network/timeout errors (no response) propagate raw.

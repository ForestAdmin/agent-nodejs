...
//
// If the MCP server gains/loses an @forestadmin/* dependency, both must change or the
// runtime image breaks at startup (Cannot find module) — invisible to a build-only CI.
// This check recomputes the real transitive @forestadmin closure of mcp-server
...
const ROOT_PACKAGE = '@forestadmin/mcp-server';
...
// Every closure package except the MCP server itself must be copied into node_modules.
for (const pkg of actual) {
  if (pkg === 'mcp-server') continue;

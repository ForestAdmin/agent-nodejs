 ...
 *
 * Implementations are provided by the host application (S3, GCS, local disk...).
 * The MCP server never sees the file bytes during upload. Clients PUT them straight
 * to the URL returned by createUploadUrl, and the server only reads them back when
 * an executeAction call redeems the handle.
 */
export interface UploadStorage {
  /** Return a pre-authorized URL the client can upload a single object to. */
  createUploadUrl(params: {
    /** Storage key the object must land under. */
    key: string;
    mimeType: string;
  ...
  /**
   * Optional size probe used to reject oversized uploads without downloading them.
   * Return undefined when the size cannot be known cheaply.
   */
...
export interface FileUploadsOptions {
  ...
  /**
   * Lifetime of the file handle. Defaults to 45 minutes, longer than the upload URL,
   * so a slow upload still leaves time to run the action.
   */
  handleTtlSeconds?: number;
  /** Maximum uploaded file size, enforced when the handle is redeemed. Defaults to 20 MiB. */
  maxBytes?: number;
  /**
   * Maximum handle redemptions running at once per process. Each redemption may hold up
   * to maxBytes plus its base64 copy in memory, so this bounds the worst case to
   * maxBytes * maxConcurrentDownloads. Defaults to 5.
   */
  maxConcurrentDownloads?: number;
}

/** FileUploadsOptions with defaults applied and the auth secret attached. */
export interface ResolvedFileUploads {
  ...
  maxBytes: number;
  authSecret: string;
  /** Runs a redemption inside the process-wide concurrency bound. */

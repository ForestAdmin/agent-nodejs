...
import { UPLOADED_FILE_PREFIX, signUploadHandle } from './handles';

const MIME_TYPE_PATTERN = /^[\w.+-]+\/[\w.+-]+$/;
const SHA256_HEX_PATTERN = /^[0-9a-f]{64}$/i;
const SHA256_BASE64_PATTERN = /^[A-Za-z0-9+/]{43}=$/;

// The data URI format delimits the filename with ';' and ',', and the storage key with
// '/'. Keep a conservative charset so neither parser can be confused.
...
// Accepts the digest as hex (shasum -a 256 output) or base64. Returns base64,
// null when the digest is absent, or false when it is malformed.
 ...
 *
 * Must be mounted behind requireBearerAuth so req.auth carries the caller's identity.
 * The returned handle is bound to that user and can only be redeemed by them.
 */

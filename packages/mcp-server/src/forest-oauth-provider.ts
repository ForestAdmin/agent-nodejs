...
export default class ForestOAuthProvider implements OAuthServerProvider {
   ...
   * This is the framework-agnostic version that returns a URL string.
   *
   * @returns The URL to redirect to, or an error redirect URL prefixed with "error:"
   */
  async getAuthorizationUrl(
    ...
    try {
      ...
      // Build redirect URL to Forest Admin agent for actual authentication
      ...
      return agentAuthUrl.toString();
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger('Error', `[ForestOAuthProvider] Authorization error:: ${errorMessage}`);

      // Return error redirect URL prefixed with "error:" to indicate it's an error redirect
      const errorUrl = `${params.redirectUri}?error=server_error&error_description=${encodeURIComponent(
  ...
  /**
   * Handle authorization request with Express response object.
   * This is kept for backwards compatibility with the Express server.
   */
  async authorize(
    client: OAuthClientInformationFull,
    params: AuthorizationParams,
    ...
    const url = await this.getAuthorizationUrl(client, params);

    if (url.startsWith('error:')) {
      res.redirect(url.slice(6));
    } else {
      res.redirect(url);
    }
  }

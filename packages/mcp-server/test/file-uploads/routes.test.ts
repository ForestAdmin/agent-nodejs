...
function makeApp(options: { storage?: UploadStorage; auth?: AuthInfo }): {
  app: Express;
  ...
  const uploads = resolveFileUploads({ storage }, AUTH_SECRET);
...
function claimsOf(fileHandle: string) {
  return verifyUploadHandle(fileHandle.slice('$uploadedFile:'.length), 42, AUTH_SECRET);
}
...
describe('POST /files', () => {
  ...
  it('returns an upload destination and a redeemable handle', async () => {
    ...
    expect(claimsOf(response.body.fileHandle)).toMatchObject({
      name: 'report.pdf',
      mimeType: 'application/pdf',
    });
  });
  ...
  it('keeps a filename the agent can round-trip, spaces and all', async () => {
    ...
    const response = await request(app)
      .post('/files')
      .send({ filename: 'rapport final (v2).pdf', mimeType: 'application/pdf' });

    expect(claimsOf(response.body.fileHandle).name).toBe('rapport final (v2).pdf');
  });

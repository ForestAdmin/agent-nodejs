...
- Each `src/decorators/<feature>/` folder is one layer, typically a `collection.ts` (extends a toolkit `CollectionDecorator`) plus a `datasource.ts` and `types.ts`. The stack exposes named handles so builder methods reach the right layer. The computed→relation→computed sandwich is split into two handles, `this.earlyComputed` and `this.lateComputed` (there is no `this.computed`); other layers are single handles like `this.action`, `this.relation`.

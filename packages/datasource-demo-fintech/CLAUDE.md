...
- **A new database is re-rolled on every boot** — `Math.random` is unseeded, so the dataset differs each run. `index.ts` does `rm('db.sqlite', { force: true })` before seeding; the second-boot test exists to guarantee `createTable` never fails on a leftover file. Don't write tests that assume specific seeded values.

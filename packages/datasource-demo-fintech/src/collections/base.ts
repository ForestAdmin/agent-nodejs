...
export default class BaseFintechCollection extends BaseCollection {
  ...
  constructor(datasource: DataSource, name: string, fields: Record<string, FieldSchema>) {
    ...
    // filters/sort is supported on every column
    for (const schema of Object.values(this.schema.fields)) {
      if (schema.type === 'Column') {
        schema.filterOperators = BaseFintechCollection.supportedOperators;
  ...
  async create(caller: Caller, data: RecordData[]): Promise<RecordData[]> {
    ...
    for (const datum of data) {
      const maxId = this.records.reduce((id, r) => (id < r.id ? (r.id as number) : id), 0);

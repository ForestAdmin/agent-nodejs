...
**Sync triggers** flow from `SyncCollectionDecorator` (src/decorators/sync/collection.ts): list/aggregate enqueue `before-*` pull-deltas when `pullDeltaOnBeforeAccess`; create/update/delete enqueue `after-*` pull-deltas when `pullDeltaOnAfterWrite`. Writes are wired to the user's `createRecordHandler`/`updateRecordHandler`/`deleteRecordHandler` by `WriteCollectionDecorator` (a missing handler makes that op throw `UnprocessableError`). ⚠️ Flattening and write handlers are **mutually exclusive**: `WriteDataSourceDecorator`'s constructor throws if any `flattenOptions` (`asModels`/`asFields`) coexist with any write handler — a replica is either writable *or* flattened, never both.
